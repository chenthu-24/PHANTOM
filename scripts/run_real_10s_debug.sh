...
ART="${1:-artifacts/real_10s_debug}"
...
cleanup_nodes() {
  local pids
  pids=$(ps aux | awk '/ros_robot_controller|odom_publisher|usb_cam_node_exe|detector_node|free_space_node|rear_perception_node|planner_controller_node|safety_shield_node|sllidar_node|ydlidar_ros2_driver_node|ldlidar_stl_ros2_node/ && !/awk/ {print $2}')
  if [ -n "$pids" ]; then
...
if [ "$ready" -ne 1 ]; then
  ...
  cp "$ART/topic_list_latest.txt" "$ART/topic_list_during_launch.txt" 2>/dev/null || timeout 3 ros2 topic list > "$ART/topic_list_during_launch.txt" 2>&1 || true
  stop_robot
...
python3 scripts/record_debug_topics.py "$ART" --duration 12.0 > "$ART/recorder_stdout.txt" 2>&1 &
recorder_pid=$!

(
  ros2 topic list > "$ART/topic_list_during_launch.txt" 2>&1 || true
  for topic in /scan /nav/front_free_space /nav/rear_risk /det/detections /cmd_vel_raw /phantom/disabled_cmd_vel /controller/cmd_vel /odom /odom_raw /debug/planner_state /debug/safety_decision; do
    ...
    ros2 topic info "$topic" -v >> "$ART/topic_info_during_launch.txt" 2>&1 || true
  done
) &

python3 scripts/relay_cmd_vel_10s.py --source /phantom/disabled_cmd_vel --dest /controller/cmd_vel --duration 10.0 > "$ART/relay_stdout.txt" 2>&1 || true
